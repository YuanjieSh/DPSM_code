...
def smooth_aps_score_all(
  ...
  def smooth_sort_fn(p, d):
    ...
    # This is done as, without randomization, we want the class
    # that just exceeds the threshold, to be included in the confidence set.
    matrix_l = jnp.triu(jnp.ones((p.shape[0], p.shape[0]), dtype=jnp.float32))
    matrix_l = matrix_l.at[jnp.diag_indices(matrix_l.shape[0])].set(d)

    _, cum_sorted_p = forward_fn(-p, p, matrix_l)
    _, cum_p, _ = forward_backward_fn(-p, cum_sorted_p)
    return cum_p

  # probabilities_np = jnp.array(probabilities.detach().cpu().numpy())
  probabilities_np = jax.device_put(probabilities.detach().cpu().numpy()).astype(jnp.float32)

  if rng is not None:
    diagonals = jax.random.uniform(rng, (probabilities_np.shape[0],))
  else:
    diagonals = jnp.zeros(probabilities_np.shape[0], dtype=jnp.float32)
  smooth_sort_vmap = jax.vmap(smooth_sort_fn, (0, 0), 0)
  scores_all = smooth_sort_vmap(probabilities_np, diagonals)
  # scores_all_tensor = torch.tensor(scores_all, device=device, dtype=probabilities.dtype) 
  scores_all_tensor = torch.tensor(np.array(scores_all), device=device, dtype=torch.float32)
  return scores_all_tensor

def smooth_aps_score(
    probabilities: jnp.ndarray,
    labels: jnp.ndarray,
    sos: variational_sorting_net.VariationalSortingNet,
    device: torch.device,
  ...
  # probabilities_np = jnp.array(probabilities.detach().cpu().numpy())
  probabilities_np = jax.device_put(probabilities.detach().cpu().numpy()).astype(jnp.float32)
  # labels_np = jnp.array(labels.detach().cpu().numpy())
  labels_np = jax.device_put(labels.detach().cpu().numpy()).astype(jnp.int32)

  if rng is not None:
    diagonals = jax.random.uniform(rng, (probabilities_np.shape[0],))
  else:
    diagonals = jnp.ones(probabilities_np.shape[0], dtype=jnp.float32)
  smooth_sort_vmap = jax.vmap(smooth_sort_fn, (0, 0, 0), 0)
  scores = smooth_sort_vmap(probabilities_np, diagonals, labels_np)
  scores_tensor = torch.tensor(np.array(scores), device=device, dtype=torch.float32)
